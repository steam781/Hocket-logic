...
public class Match
{
    ...
    private void PlayMinute()
    {
        ...
        if (random.Next(5) == 0)
        {
            Console.WriteLine("Shot on goal!");

            // Check if the shot results in a goal
            int scoringTeam = -1;
            if (random.Next(10) < (teamWithPuck == 0 ? homeLPE : awayLPE))
            {
                Console.WriteLine("GOAL!!!");
                scoringTeam = teamWithPuck;
            }
        }
    ...
    {
        if (attackingTeam.LPE > defendingTeam.LPE)
        {
            ...
            {
                Console.WriteLine("Shot on target, but {0} saves it.", defendingTeam.Name);
            }
        }
        else
        {
